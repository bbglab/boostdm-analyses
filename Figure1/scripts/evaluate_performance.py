...
def plot_contingency(confusion_summary, csqn_type, moa, keyword, role_dict, gene_specific_models, outpath):
    ...
    ax[0] = sns.boxplot(data=true_boostdms, showfliers=False, color='white',
                        medianprops={'color': 'black'}, linewidth=0.4, ax=ax[0])
    
    ax[0].scatter(x, y, s=3, alpha=0.25, color='DarkRed')
    ...
    ax[0].hlines(0.5, -0.5, len(true_boostdms)+0.5, lw = 0.5, color = 'grey')  # ls='--'
    ...
    ax[1].scatter(x, y, s=3, alpha=0.25, color='grey')
    ...
    ax[1].hlines(0.5, -0.5, len(false_boostdms)+0.5, lw=0.5, color='grey')  # ls='--', 
    ...
    plt.savefig(f'{outpath}/comparison.genewise.{keyword}.{moa}.{csqn_type}.validation.png',
...
def plot_summary_contingency(fscore, recall, precision, true_boostdms, false_boostdms, keyword, moa, outpath, csqn_type):
    ...
    fig, ax = plt.subplots(figsize=(2.5, 1.25), nrows=1, ncols=2, gridspec_kw={'width_ratios': [1, 0.5]})
    ...
    ax[0] = sns.boxplot(data=[true_boostdms, false_boostdms], showfliers=False,
                        color='white', medianprops={'color': 'black'}, linewidth=0.7, ax=ax[0])
    ax[0].hlines(0.5, -0.5, 1.5, lw=0.5, color='grey')  # ls='--', 
...
def plot_boostdm_validation_bias(df, true_label, false_label, csqn_type, moa, key, role_dict, gene_specific, outpath):
    ...
    # cross comparison 
    cross_comparison_dict, cross_comparison_dict_full = get_cross_comparison(enrichment_dict, true_label, false_label)
    ...
    fscore, recall, precision, true_boostdms, false_boostdms = plot_contingency(confusion_summary, csqn_type, moa,
                                                                                keyword, role_dict, gene_specific,
                                                                                outpath)
    
    plot_summary_contingency(fscore, recall, precision, true_boostdms, false_boostdms,
                             keyword, moa, outpath, csqn_type)
...
def comparison_main(input, outpath, true_label, false_label, csqn_type, label,  predictions):
    ...
    path_saturation_prediction = predictions
    ...
    for moa in ['LoF', 'Act']:    
        ...
        acc, ppv, npv = plot_boostdm_validation_bias(df, true_label, false_label, csqn_type, moa, label, role_dict,
                                                     gene_specific, outpath)
        res[moa] = (acc, ppv, npv)
